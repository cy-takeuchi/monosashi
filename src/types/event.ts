 ...
 * 行を削除しても何も飛ばない。
 *
 * ハンドラを 270 件（テーブルのコードを含む）登録した状態で測っているので、
 * 「登録漏れで聞こえなかった」ではない（実測 2026-08-31）。
 *
 * UI 操作の行は**手動で確認したもので、フィクスチャには含まれない**。
 * 再現には kintone 内部の DOM を操作する必要があり、採取の方針と衝突するため。
 * 型はテンプレートリテラルで両方のイベント名を受けるので、実害はない。
 *
