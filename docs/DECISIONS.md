...
### 「例外が出なかった」を受け入れの根拠にしない

`set()` が失敗したときに **JS の例外として上がってくるかが分かっていない**。
分かっているのは、`type` を省くと kintone が
「カスタマイズ用の JavaScript の実行時にエラーが発生しました」を出すことだけ
...
**測るときはこれが効く。** `nullableString` の 3 種別に `null` を渡すケースを
1 つだけにすると、「同じ形だから同じだろう」という推測が入る。
3 つ揃えれば 1 回の実測で

- 揃って通る / 揃って落ちる → **変換を形ごとに書ける**
- 揃わない → **種別ごとに書くしかない**

が決まる。`setCases.test.ts` が
「`null` を受け付ける全種別に `null` を渡している」ことを縛っている。
判定は `VALUE_SHAPE` を export せず `canSetValue` の挙動から引く
（内部の表を公開 API にしないため）。
