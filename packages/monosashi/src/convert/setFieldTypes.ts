 ...
 * 「除くべき」を仕様の推測で決めず、実際に渡した結果で決めている。
 *
 ...
 *
 * 除くのは正しさの要件ではなく整形。
 * 残っていても黙って無視されるので、除き漏れがあっても壊れない。
 */
...
/**
 * `set()` への書き込みで除くべき type かどうか。
 *
 * `record[code]?.type` をそのまま渡せるよう undefined も受け取る。
 */
export const isExcludedOnSet = (type: string | undefined): boolean =>
	type !== undefined && (rejected.has(type) || ignored.has(type));

/**
 * 除かないと `set()` がエラーを出す type かどうか。
 *
 * ```ts
