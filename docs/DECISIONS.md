...
## change イベントは操作の種類で名前が変わる

**実測 2026-08-31。** ハンドラを 270 件（テーブルのコードを含む）登録した状態で測った。

| 操作 | イベント名 | changes.row |
| --- | --- | --- |
| 表外のフィールドを変更 | `change.<フィールドコード>` | `null` |
| 表内のセルを変更 | `change.<表内フィールドのコード>` | 行 |
| `set()` で行を追加 | `change.<表内フィールドのコード>` | 行 |
| `set()` で行を削除 | **発火しない** | — |
| UI で行を追加・削除 | `change.<テーブルのコード>` | 未測定 |

**テーブルのコードで飛ぶのは UI 操作のときだけ。**
`set()` で表を部分更新しても飛ばない。行を足したときに飛ぶのは
新しい行のセルに値が入るからで、表そのものの変化としてではない。

UI 操作の行は手動で確認したもので、フィクスチャには含まれない。
再現には kintone 内部の DOM を操作する必要があり、採取の方針と衝突する。
型はテンプレートリテラルで両方を受けるので実害はない。

### この測定に至るまでに 2 回間違えた

**1 回目**: 「行の追加・削除はテーブルのコードで飛ぶ」という仮説を立て、
`set()` で測って飛ばなかったので仮説が外れたと結論した。
実際には `set()` と UI で挙動が違うだけで、仮説は UI については正しかった。

**2 回目**: 「飛ばなかった」と報告したが、
**テーブルのコードのハンドラを登録していたかを確認していなかった**。
登録漏れなら「飛ばなかった」ではなく「聞いていなかった」で、まったく別の話になる。

どちらも**測っていないことを測ったことにした**のが原因。対策を 2 つ入れた。

- 登録した change イベント名を probe が保持し、**測定前に e2e が確認する**。
  テーブルのコードのハンドラが無ければ、結果を出す前に落ちる
- 発火した**イベント名そのもの**を記録する。
  以前は回数しか記録しておらず、何が飛んだかは前後のサンプルからの推測だった
