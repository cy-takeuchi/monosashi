...
採取カスタマイズの適用（`updateAppCustomize` / `deployApp`）で
kintone が一過性の 500（`GAIA_CS02`「予期せぬエラーが発生しました」）を返すことがある。実際に踏んだ。

そのとき適用が中途半端に終わり、続く採取が「パネルが出ない」で失敗する。
**kintone が変わったわけではないのに週次ジョブが赤くなる。**
この仕組みの価値は「落ちたら必ず本物の変化」であることなので、偽陽性は避ける。

適用ステップを 3 回までリトライする。
採取そのものは Playwright の `retries: 1`（CI のみ）が拾う。
毎回新しいレコードを作るので、リトライしても状態は汚れない。
