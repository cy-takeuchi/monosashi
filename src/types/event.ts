 ...
 * **`<prefix>.change.<表内フィールドのコード>`**（実測 2026-08-30）。
 * テーブルのコードではない。`changes.field` も SUBTABLE 全体ではなく
 * **変更されたセルそのもの**が入る。
 *
 * | 変更した場所 | イベント名 | changes.field | changes.row |
 * | --- | --- | --- | --- |
 * | 表内のセル | `change.t_singleLineText` | セル | `{ id, value }` |
 * | 表外のフィールド | `change.singleLineText` | フィールド | `null` |
 *
 * つまり `changes.row` が非 null かどうかが、表内の変更かどうかの判別になる。
 */
