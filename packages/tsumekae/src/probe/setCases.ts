 ...
 *
 * `toSetRecord`（未実装）が何を除き、何を変換すべきかは、この結果が根拠になる。
 * 推測で「読み取り専用は除くべき」と決めるのではなく、実際に渡して確かめる（#14）。
 *
 ...
 * | 向き | 何が問題になるか |
 * |---|---|
 * | `get()` → `set()` | JS API 同士なので値の表現は揃っている。除く種別だけが論点 |
