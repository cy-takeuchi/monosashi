 ...
 * ## 同じ操作でも経路でイベント名が違う
 *
 * 実測 2026-08-31。作成画面・編集画面の両方で一致。
 *
 * | 操作 | UI 経由 | `kintone.app.record.set()` 経由 |
 * | --- | --- | --- |
 * | 表外のフィールドを変更 | 測定不能 | `change.<フィールドコード>` |
 * | 表内のセルを変更 | 測定不能 | `change.<表内フィールドのコード>` |
 * | 行の追加 | **`change.<テーブルのコード>`** | `change.<表内フィールドのコード>` |
 * | 行の削除 | **`change.<テーブルのコード>`** | **発火しない** |
 *
 * 行の追加が典型で、**UI ならテーブルのコード、`set()` なら表内フィールドのコード**で飛ぶ。
 * `set()` での行削除は何も飛ばない。
 *
 * ハンドラを 270 件（テーブルのコードを含む）登録した状態で測っており、
 * 「登録漏れで聞こえなかった」ではない。
 *
 * UI での値入力が測定不能なのは、kintone のフィールド入力欄に
 * accessible name が無く、役割と名前で掴めないため。
 * 内部セレクタを使えば可能だが、kintone の更新で壊れるので採らない。
 * なお UI での値入力は**フォーカスを外したときに**飛ぶ
