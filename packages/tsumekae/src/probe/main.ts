...
import { rememberFileValues, runSetCase } from "./runSetCase.js";
...
on(EVENTS_WITH_RECORD, (event) => {
	...
	// FILE の中身を覚えておく。get() では取れないため（runSetCase.ts）
	rememberFileValues(
...
const api = {
	...
	runSetCase: (id: string): boolean => runSetCase(id, sampleScreen()),
