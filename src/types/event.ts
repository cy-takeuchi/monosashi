 ...
 * | 操作 | UI 経由 | `set()` 経由 |
 * | --- | --- | --- |
 * | 表外のフィールドを変更 | `change.<フィールドコード>` | 同じ |
 * | 表内のセルを変更 | `change.<表内フィールドのコード>` | 同じ |
 * | 行の追加 | **`change.<テーブルのコード>`** | **`change.<表内フィールドのコード>`** |
 * | 行の削除 | **`change.<テーブルのコード>`** | **発火しない** |
 *
 * **値の変更は経路によらず同じ。違うのは行操作だけ。**
 *
 * 行を扱うプラグインは、UI 操作を捕まえるならテーブルのコードで、
 * `set()` の結果を捕まえるなら表内フィールドのコードで待つ必要がある。
 * `set()` での行削除は何を待っても飛ばない。
 *
 * ハンドラを 270 件（テーブルのコードを含む）登録した状態で測っており、
 * 「登録漏れで聞こえなかった」ではない。
 *
 * UI での値変更は**フォーカスを外したときに**飛ぶ。
 *
